        ...
        <Table>
            ...
            <Row ss:Index="4" ss:AutoFitHeight="1">
                ...
                <Cell ss:Index="2" ss:StyleID="Header">
                    <Data ss:Type="String">VALOR</Data>
                </Cell>
            ...
            <Row ss:Index="<?= $i + 5; ?>">
                <Cell ss:Index="1">
                    <Data ss:Type="Number"><?= (Int)($reporte[$i]['id_producto'] / 10); ?></Data>
                </Cell>
                <Cell ss:Index="2">
                    <Data ss:Type="String"><?php 
                            $valor = $reporte[$i]['id_producto'] % 10;
                            switch ($valor) {
                                case 1: 
                                    echo 'IMPUTADO';
                                    break;
                ...
                for ($j = $n; $j < count($keys); $j ++) {
                    if ($reporte[$i][$keys[$j]] == '' || ($reporte[$i]['id_producto'] % 10 == 2)) {
                        $celda = explode('|', $reporte[$i][$keys[$j]]);
                        if ($celda[0] == 20) {
                            echo '<Cell ss:Index="'.($j + 14 - $n).'" ss:StyleID="Precio">'."\r\n";
                            echo '<Data ss:Type="Number">'.(Float)$celda[1]."</Data>\r\n";
                            echo "</Cell>\r\n";
                        }
                    } else {
                        $celda = explode('|', $reporte[$i][$keys[$j]]);
                        if ($reporte[$i]['id_producto'] % 10 == 3) {
                            echo '<Cell ss:Index="'.($j + 14 - $n).'" ss:StyleID="PrecioGray">'."\r\n";
                            echo '<Data ss:Type="Number">'.(Float)$celda[1]."</Data>\r\n";
                            echo "</Cell>\r\n";
                        } else {
                            if ($celda[0] == 20) {
